...
def load_data(path):
    if path.startswith('data-proj01/'):
        s3  = get_s3()
        obj = s3.get_object(Bucket='data-proj01', Key=path.replace('data-proj01/', ''))
...
def export_onnx(model, vocab, run_name):
...
def save_and_register(model, vocab, config, metrics, run_name):
    ...
    onnx_path = export_onnx(model, vocab, run_name)
    try:
        s3 = get_s3()
        with open(ckpt_path, 'rb') as f:
            s3.put_object(Bucket='data-proj01',
                Key=f'models/checkpoints/subst_model_v{run_id}.pth', Body=f)
        with open(ckpt_path, 'rb') as f:
            s3.put_object(Bucket='data-proj01',
                Key='models/production/subst_model_current.pth', Body=f)
        with open(onnx_path, 'rb') as f:
            s3.put_object(Bucket='data-proj01',
                Key='models/production/subst_model_current.onnx', Body=f)
        with open(vocab_path, 'rb') as f:
            s3.put_object(Bucket='data-proj01',
                Key='models/production/vocab.json', Body=f)
        print('All files uploaded to data-proj01/models/')
    except Exception as e:
        print(f'Object storage skip: {e}')
    #    mlflow.pytorch.log_model(model, 'model')
    #    mlflow.register_model(f'runs:/{run_id}/model', 'subst-model-candidate')
