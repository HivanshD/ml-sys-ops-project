...
def load_model():
    ...
    try:
        ...
        print(f"[startup] ONNX loaded. Inputs: "
              f"{[i.name for i in _session.get_inputs()]}")
        _warmup()
        MODEL_LOADED.set(1)
    except Exception as e:
...
def infer(context_ids, missing_id, top_k=3):
    ...
    return [
        {"ingredient": _id_to_ingredient.get(int(i), f"<id_{i}>"),
         "rank": r, "embedding_score": round(float(max(0, min(1, scores[i]))), 4)}
...
def _infer_stub(context_ids, missing_id):
    """Matches training's forward: .mean(dim=1) + cosine_similarity, no padding mask."""
...
def tokenize_ingredients(ingredient_strings):
    ids = [_vocab.get(s.lower().strip(), UNK_ID)
           for s in ingredient_strings if isinstance(s, str)]
    ids = ids[:CONTEXT_LEN]
